...
func waitUntil(when time.Time) {
	...
	if sleepDuration > 0 {
		...
		log("Waiting for", int(sleepDuration.Seconds()), "seconds")
		watchAndWait(int(sleepDuration.Seconds()))
	} else {
		log("Time has already started or passed, proceeding immediately.")
	}
}

func nextScheduledTransmissionFrom(now time.Time) time.Time {
